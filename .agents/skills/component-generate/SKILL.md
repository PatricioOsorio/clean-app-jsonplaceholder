...
description: >
  Scaffolds React components, compound components, hooks, pages (multi-step forms & listing/tables), and host layout components following
  ...
  e.g. `CardDashboard.Skeleton`), logic-in-hooks (`use-<name>.config`), the
  form-builder pattern for pages (`createFormConfig` + `useFormBuilder`), and the listing/table page pattern (`filters-form`, `<domain>-table`, `use-<name>-page`). Use whenever
  the user asks to create, scaffold, add, or generate a new React component, compound
  component, sub-component, form, page, or layout component in any MFE of the simba monorepo — or in
...
`steppper-preticket`, `new-preticket-page`, `folios-page`, `my-pretickets-page`, `aside`, `header`, `footer`) — not invented. Follow these
...
2. **Shared typing comes from `lib-styleguide-simba/interfaces`** — `IWithRootProps<T>`,
   ...
   `IWithError`/`IWithNoMatch`/`IStatusContentProps`, `IWithTestId`. Read `references/interfaces-and-typing.md` before
   writing any `.interfaces.ts` file — guessing the shape of `IWithRootProps` instead
   of reading it is the single most common mistake. Use `<StatusContent>` from
   `lib-styleguide-simba/status-content` to wrap conditional async states (`isLoading`, `isError`, `noMatch`, `isEmpty`) in components — see `references/patterns.md` §5.
...
- [ ] Props interface extends `IWithRootProps<T>` (or the right `IWith*` mixin / `IStatusContentProps`) from
      `lib-styleguide-simba/interfaces` — not reinvented locally.
- [ ] Conditional loading/error/empty/noMatch states wrapped with `<StatusContent>` when applicable.
